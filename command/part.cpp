...
void PART2(Client &sender, std::string message, Channel &channel)
{
    for (std::vector<Client *>::iterator itUser = channel.users.begin(); itUser != channel.users.end(); itUser++)
    {
	...
	if(ite.commands.size() == 3)
	{
		...
		for(std::vector<Channel>::iterator itChannels = channels.begin(); itChannels != channels.end(); itChannels++)
		{
			if(k[1] == itChannels->ChannelName)
			{
				if(channels.size() >= 1)
				{
					for(std::vector<Client *>::iterator users1 = itChannels->users.begin(); users1 != itChannels->users.end(); users1++)
					{
